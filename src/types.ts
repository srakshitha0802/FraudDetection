...
export interface UserProfile {
  ...
  usual_transaction_times: string[];
...
export interface AnalyticsData {
  ...
  category_breakdown: { category: string; count: number; share: string }[];
  hourly_trend: { hour: string; legitimate: number; fraud: number }[];
}
...
export interface ModelMetricsData {
  ...
  feature_importances: { name: string; score: number; description: string }[];
...
export interface AuthUser {
  ...
  phone?: string;
  avatar?: string;
  role: 'LEAD_INVESTIGATOR' | 'COMPLIANCE_OFFICER' | 'RISK_ENGINEER' | 'FRAUD_ANALYST' | 'PERSONAL_USER';
  roleTitle: string;
  clearanceLevel: 'LEVEL_3_ADMIN' | 'LEVEL_2_SENIOR' | 'LEVEL_1_OPERATOR' | 'PERSONAL';
  provider: 'google' | 'email' | 'sso';
  lastLogin: string;
...
export interface PoliceComplaint {
  complaint_id: string;
  acknowledgement_number: string;
  national_portal_ref: string;
  filed_at: string;
  victim_name: string;
  victim_email: string;
  victim_phone: string;
  victim_address: string;
  incident_category: 'UPI_FRAUD' | 'CARD_SKIMMING' | 'OTP_PHISHING' | 'UNAUTHORIZED_DEBIT' | 'IDENTITY_THEFT' | 'EXTORTION_SCAM';
  incident_date: string;
  total_fraud_amount: number;
  currency: string;
  police_station: string;
  police_jurisdiction: string;
  associated_transaction_ids: string[];
  suspect_details: {
    suspect_upi?: string;
  ...
  fir_status: 'SUBMITTED' | 'ACKNOWLEDGED' | 'OFFICER_ASSIGNED' | 'BANK_LIEN_PLACED' | 'RESOLVED_RECOVERED';
  investigating_officer: string;
  officer_badge: string;
  officer_contact: string;
  evidence_summary: string;
  formal_narrative: string;
  recovery_stage: string;
}

export interface PersonalFraudAlert {
  alert_id: string;
  transaction_id: string;
  timestamp: string;
  amount: number;
  ...
  risk_score: number;
  threat_type: 'CARD_CLONING_SUSPECTED' | 'UNKNOWN_DEVICE_LOGIN' | 'UNUSUAL_GEO_LOCATION' | 'HIGH_VELOCITY_DEBIT' | 'MULE_BENEFICIARY' | 'PHISHING_ATTEMPT';
  severity: 'CRITICAL' | 'HIGH' | 'MEDIUM';
  status: 'ACTION_REQUIRED' | 'COMPLAINT_FILED' | 'CARD_BLOCKED' | 'RESOLVED_SAFE';
  explanation: string;
  recommended_action: string;
...
export interface EmailDispatchRecord {
  ...
  bodyText: string;
  bodyHtml: string;
  category: 'AUTH_OTP' | 'CRITICAL_FRAUD_ALERT' | 'CARD_BLOCKED' | 'POLICE_COMPLAINT' | 'MODEL_DEPLOYED' | 'TEST_PING';
